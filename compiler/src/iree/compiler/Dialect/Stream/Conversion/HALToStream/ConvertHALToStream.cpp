...
// ->
// %r0b, %t0 = stream.timepoint.signal %r0a :
//                 tensor<4xf32> in !stream.resource<*> => !stream.timepoint
// %r1b, %t1 = stream.timepoint.signal %r1a :
...
struct ConvertTensorBarrierOp
  ...
  LogicalResult matchAndRewrite(
    ...
    for (auto sourceResource : adaptor.getSources()) {
      ...
      auto signalOp = rewriter.create<IREE::Stream::TimepointSignalOp>(
          sourceResource.getLoc(), source.resource.getType(), timepointType,
          source.resource, source.resourceSize, /*affinity=*/nullptr);
      signaledResources.push_back(signalOp.getResult());
      signaledTimepoints.push_back(signalOp.getResultTimepoint());
    }
    Value joinedTimepoint = rewriter.create<IREE::Stream::TimepointJoinOp>(
        op.getLoc(), timepointType, signaledTimepoints);
    rewriter.create<IREE::Stream::TimepointExport2Op>(
