...
// Finds a !hal.fence bound to |timepoint| via an export2 op and returns it if
// it is usable at the builder insertion point. The export2 ops is only used if
...
static Value consumeBoundFence(Value timepoint,
  ...
  auto export2Op =
      dyn_cast<IREE::Stream::TimepointExport2Op>(*timepoint.getUsers().begin());
  if (!export2Op) return nullptr;  // non-export use
  assert(!export2Op.getExternalValues().empty());
  auto fence = export2Op.getExternalValues().front();
...
struct TimepointExport2OpPattern
    : public StreamConversionPattern<IREE::Stream::TimepointExport2Op> {
  using StreamConversionPattern::StreamConversionPattern;
  LogicalResult matchAndRewrite(
...
struct TimepointSignalOpPattern
    : public StreamConversionPattern<IREE::Stream::TimepointSignalOp> {
  using StreamConversionPattern::StreamConversionPattern;
  LogicalResult matchAndRewrite(
    ...
    Value nullFence = rewriter.create<IREE::Util::NullOp>(
        signalOp.getLoc(), rewriter.getType<IREE::HAL::FenceType>());
    rewriter.replaceOp(signalOp, {adaptor.getResource(), nullFence});
    return success();
  }
};
