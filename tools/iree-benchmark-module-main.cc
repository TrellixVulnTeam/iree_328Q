...
IREE_FLAG(int32_t, concurrency, 1,
...
static void BenchmarkAsyncFunction(const std::string& benchmark_name,
                                   int32_t batch_size, int32_t concurrency,
  ...
  batch_size = (int32_t)iree_host_align(batch_size, concurrency);
  ...
  while (state.KeepRunningBatch(batch_size)) {
    ...
    IREE_CHECK_OK(
        iree_hal_fence_create(concurrency, host_allocator, &completion_fence));
    for (int32_t i = 0; i < batch_size / concurrency; ++i) {
      for (int32_t j = 0; j < concurrency; ++j) {
        ...
        if (i > 0) {
          wait_fence =
              vm::retain_ref(invocation_fences[(i - 1) * concurrency + j]);
        }
...
void RegisterAsyncBenchmark(const std::string& function_name,
  ...
  auto benchmark_name = "BM_" + function_name;
  int32_t batch_size = FLAG_batch_size;
  int32_t concurrency = FLAG_concurrency;
  benchmark::RegisterBenchmark(benchmark_name.c_str(),
                               [=](benchmark::State& state) -> void {
                                 BenchmarkAsyncFunction(
                                     benchmark_name, batch_size, concurrency,
